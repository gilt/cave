...
class ListTeamMetricsApiSpec extends PlaySpecification with Results with AbstractMetricsApiSpec with UserData with GetMetricData {

  "GET /organizations/:name/teams/:team/metrics-list" should {
    ...
    "retrieve some metrics for an existing organization" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().listTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics-list")
