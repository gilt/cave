...
class GetOrganizationMetricsApiSpec extends PlaySpecification with Results with AbstractMetricsApiSpec with UserData with GetMetricData {

  "GET /organizations/:name/metrics" should {
    ...
    "retrieve some metrics for an existing organization" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?metric=$MetricNameOrders&aggregator=sum&period=1")
    ...
    "return 200 after handling a single tag" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?metric=$MetricNameOrders&tags=$MetricTagsOrdersString&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 200 after handling multiple tags" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?metric=$MetricNameOrders&tags=$MetricTagsServiceString&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 200 after handling a start date" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?metric=$MetricNameOrders&start=$SomeDateString&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 200 after handling an end date" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?metric=$MetricNameOrders&end=$SomeDateString&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 200 after handling a large limit" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?metric=$MetricNameOrders&limit=$LargeLimit&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 200 after handling a small limit" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?metric=$MetricNameOrders&limit=$SmallLimit&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 400 if metric not specified" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 400 if aggregator not specified" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?metric=$MetricNameOrders&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 400 if aggregator not valid" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?metric=$MetricNameOrders&aggregator=collate&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 400 if period not specified" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?metric=$MetricNameOrders&aggregator=sum")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 400 if period not valid" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getOrganizationMetrics(GiltName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/metrics?metric=$MetricNameOrders&aggregator=sum&period=boom")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
