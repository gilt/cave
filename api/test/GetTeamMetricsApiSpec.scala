...
class GetTeamMetricsApiSpec extends PlaySpecification with Results with AbstractMetricsApiSpec with UserData with GetMetricData  {

  "GET /organizations/:name/teams/:team/metrics" should {
    ...
    "retrieve some metrics for an existing team" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?metric=$MetricNameOrders&aggregator=sum&period=1")
    ...
    "also works if user is org admin" in running(FakeApplication(withGlobal = mockGlobal)) {
      Mockito.reset(mockAwsWrapper, mockDataManager, mockInfluxClientFactory)
      when(mockDataManager.findUserByToken(Matchers.eq(SOME_TOKEN), any[DateTime])
        (any[ExecutionContext])).thenReturn(Future.successful(Some(SOME_USER)))
      when(mockDataManager.getOrganization(GiltName)).thenReturn(Success(Some(GiltOrg)))
      when(mockDataManager.getTeam(GiltOrg, GiltTeamName)).thenReturn(Success(Some(GiltTeam)))
      when(mockDataManager.getTeamsForUser(Matchers.eq(GiltOrg), Matchers.eq(SOME_USER))(Matchers.any[ExecutionContext])).thenReturn(Future.successful(List(GiltName -> Role.Viewer)))
    ...
    "return 200 after handling a single tag" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?metric=$MetricNameOrders&tags=$MetricTagsOrdersString&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 200 after handling multiple tags" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?metric=$MetricNameOrders&tags=$MetricTagsServiceString&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 200 after handling a start date" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?metric=$MetricNameOrders&start=$SomeDateString&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 200 after handling an end date" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?metric=$MetricNameOrders&end=$SomeDateString&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 200 after handling a large limit" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?metric=$MetricNameOrders&limit=$LargeLimit&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 200 after handling a small limit" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?metric=$MetricNameOrders&limit=$SmallLimit&aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 400 if metric not specified" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?aggregator=sum&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 400 if aggregator not specified" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?metric=$MetricNameOrders&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 400 if aggregator not valid" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?metric=$MetricNameOrders&aggregator=collate&period=1")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 400 if period not specified" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?metric=$MetricNameOrders&aggregator=sum")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 400 if period not valid" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().getTeamMetrics(GiltName, GiltTeamName).apply(
        FakeRequest(GET, s"/organizations/$GiltName/teams/$GiltTeamName/metrics?metric=$MetricNameOrders&aggregator=sum&period=boom")
          .withHeaders(AUTHORIZATION -> AUTH_TOKEN))
    ...
    "return 403 if user not connected to the team nor org admin" in running(FakeApplication(withGlobal = mockGlobal)) {
      Mockito.reset(mockAwsWrapper, mockDataManager, mockInfluxClientFactory)
      when(mockDataManager.findUserByToken(Matchers.eq(SOME_TOKEN), any[DateTime])
        (any[ExecutionContext])).thenReturn(Future.successful(Some(SOME_USER)))
      when(mockDataManager.getOrganization(GiltName)).thenReturn(Success(Some(GiltOrg)))
