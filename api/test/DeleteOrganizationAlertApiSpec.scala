...
class DeleteOrganizationAlertApiSpec extends PlaySpecification with Results with AbstractAlertApiSpec with UserData {

  "DELETE /organizations/:name/alerts/:id" should {
    ...
    "respond with 204 when deletion succeeds" in running(FakeApplication(withGlobal = mockGlobal)) {
      ...
      val result = new TestController().deleteOrganizationAlert(GiltName, AlertId)(
        FakeRequest(DELETE, s"/organizations/$GiltName/alerts/$AlertId")
    ...
    "respond with 404 if alarm not found" in running(FakeApplication(withGlobal = mockGlobal)) {
      Mockito.reset(mockAwsWrapper, mockDataManager, mockInfluxClientFactory)
      when(mockDataManager.findUserByToken(Matchers.eq(SOME_TOKEN), any[DateTime])
        (any[ExecutionContext])).thenReturn(Future.successful(Some(SOME_USER)))
      when(mockDataManager.getOrganization(GiltName)).thenReturn(Success(Some(GiltOrg)))
    ...
    "respond with 403 if user is not a member or admin" in running(FakeApplication(withGlobal = mockGlobal)) {
      Mockito.reset(mockAwsWrapper, mockDataManager, mockInfluxClientFactory)
      when(mockDataManager.findUserByToken(Matchers.eq(SOME_TOKEN), any[DateTime])
        (any[ExecutionContext])).thenReturn(Future.successful(Some(SOME_USER)))
      when(mockDataManager.getOrganization(GiltName)).thenReturn(Success(Some(GiltOrg)))
